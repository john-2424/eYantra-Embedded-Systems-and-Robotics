...
#include "week4_simulation.h"						    // Header file included that contains macro definitions essential for Firebird V robot
...
void lcd_wr_char(char row, char column, unsigned char alpha_num_char)
{
	...
	sbit( lcd_control_port_reg, RS_pin );				// RS = 1 --- Data Input
	cbit( lcd_control_port_reg, RW_pin );				// RW = 0 --- Writing to LCD
	sbit( lcd_control_port_reg, EN_pin );				// Set Enable Pin
	_delay_ms(5);										// Delay
	cbit( lcd_control_port_reg, EN_pin );				// Clear Enable Pin
...
/*
 * Function Name: lcd_wr_custom_char
 * Input: hexa => hexadecimal type data to be stored in DDRAM
 * Output: This function writes the data onto the address in CGRAM
 * Logic: The array of hexadecimal values are been passed to the function, to create a custom character
 * Example Call: lcd_wr_custom_char(pattern[i]);
 */
...
/*
 * Function Name: lcd_create_custom_character
 * Input: location => The location/address to be pointed in CGRAM, where the data/hexadecimal values are referenced 
 *        pattern  => The array of hexadecimal values, for creating a new custom character
 * Output: Custom Character is created
 * Logic: First, the location is pointed/referenced in CGRAM and then the hexa values are stored in DDRAM
 * Example Call: lcd_create_custom_character(1,postive_1);
 */
void lcd_create_custom_character(unsigned char location,unsigned char *pattern)
{
	lcd_wr_command(0x40+(location*8));
	for(int i=1; i<8; i++)
	{
			lcd_wr_custom_char(pattern[i]);
	}
	lcd_wr_command(0x80);
}